...
template <typename T>
struct Node {
    Node() : data(T()), color(black), left(nullptr), right(nullptr), parent(nullptr) {}
    Node(const T &data, node_color color, Node<T> *left, Node<T> *right, Node<T> *parent)
    : data(data), color(color), left(left), right(right), parent(parent) {}
    Node(const Node<T> &node)
    : data(node.data), color(node.color), left(node.left), right(node.right), parent(node.parent) {}
